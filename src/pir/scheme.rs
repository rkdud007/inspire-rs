...
#[allow(unused_imports)]
use super::{client::*, engine::*, measurement::*, params::*};
#[allow(unused_imports)]
...
impl fmt::Display for ProtocolType {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str(match self {
            Self::SimplePIR => "SimplePIR",
            Self::DoublePIR => "DoublePIR",
...
impl FromStr for ProtocolType {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s {
            "SimplePIR" => Ok(Self::SimplePIR),
            "DoublePIR" => Ok(Self::DoublePIR),
...
pub fn run_ypir_batched(
    ...
    let (params, (resized_db_first_dim, resized_db_second_dim, resized_item_size_bits)) =
        match protocol_type {
            ...
            ProtocolType::InsPIRe => params_for_scenario_medium_payload(
                num_items,
                item_size_bits,
                gammas.clone(),
                performance_factor,
    ...
    let mut measurement = match protocol_type {
        ProtocolType::SimplePIR => run_simple_ypir_on_params(
            params,
        ...
        ProtocolType::DoublePIR => run_ypir_on_params(
            params,
            second_level_packing_mask,
    ...
    measurement.specs.resized_database_size_mb =
        (resized_db_first_dim * resized_db_second_dim * resized_item_size_bits) as f64
            / (8. * 1024. * 1024.);
...
pub fn run_simple_ypir_on_params(
    ...
    let offline_values = y_server.perform_offline_precomputation_simplepir(
        gamma,
    ...
    for trial in 0..trials + 1 {
        ...
        let target_idx: usize = (rng.random::<u64>() as usize) % (db_rows * db_cols);
        ...
        let mut packing_keys = match packing_type {
            ...
            PackingType::CDKS => PackingKeys::init_cdks(&params, sk_reg, STATIC_SEED_2),
            PackingType::NoPacking => {
        ...
        // check correctness
        for (response_switched, (y_client, target_idx, _)) in responses.iter().zip(queries.iter()) {
            ...
            for ct_bytes in response_switched.iter() {
                let ct = PolyMatrixRaw::recover_how_many(
                    &params,
                    rlwe_q_prime_1,
                    rlwe_q_prime_2,
                    gamma,
...
pub fn run_ypir_on_params(
    ...
    let start_offline_comp = Instant::now();
    let offline_values =
        y_server.perform_offline_precomputation(gamma, Some(&mut measurements[0]), online_only);
    let offline_server_time_ms = start_offline_comp.elapsed().as_millis();
    ...
    for trial in 0..trials + 1 {
        ...
        queries.push((y_client, target_idx, packed_query_row_u32, packed_query_col));
            ...
            responses.iter().zip(queries.iter())
        {
            ...
            let outer_ct = response
                .iter()
                .flat_map(|ct| {
                    decrypt_ct_reg_measured(y_client.client(), &params, &ct.ntt(), gamma).as_slice()
                        [..gamma]
                        .to_vec()
                })
                .collect::<Vec<_>>();
            ...
            for i in 0..blowup_factor.ceil() as usize {
                match second_level_packing_body {
                    ...
                    PackingType::NoPacking => {
                        val |= (outer_ct[num_rlwes_for_mask * gamma + i * gamma]
                            % params.pt_modulus)
                            << (i * pt_bits);
                    }
                }
    ...
    final_measurement.online.noise_width = mean_f64(
        &measurements
            .iter()
            .map(|m| m.online.noise_width)
            .collect::<Vec<_>>(),
    );
    let all_noise_width = measurements
        .iter()
        .map(|m| m.online.noise_width)
        .collect::<Vec<_>>();
    final_measurement.online.std_dev_server_time_ms = std_dev_f64(&all_noise_width);
...
pub fn run_ypir_with_medium_payload_on_params(
    ...
    let start_offline_comp = Instant::now();
    let offline_values = y_server.perform_offline_precomputation_medium_payload(
        &gammas,
        Some(&mut measurements[0]),
        online_only,
    );
    ...
    for trial in 0..trials + 1 {
        ...
        let query_size = (query_row_last_row.len() * params.modulus_log2 as usize
            + query_col_last_row.len() * params.modulus_log2 as usize)
            / 8;
        ...
        // check correctness
        for (response_object, (y_client, target_idx, _, _)) in responses.iter().zip(queries.iter())
        {
            let (target_row, target_col) = (target_idx / db_cols, target_idx % db_cols);
            let corr_result = y_server.get_elem(target_row, target_col).to_u64();
            ...
            assert_eq!(
                outer_ct.len(),
                mask_num_packed_rlwes * gamma_second_layer_mask
            );
                ...
                PackingType::InspiRING => {
                    ...
                    outer_ct = response_body //[mask_num_packed_rlwes..mask_num_packed_rlwes + packed_online_num]
                        ...
                        .flat_map(|ct| {
                            ...
                            .as_slice()[0..gamma_second_layer_body]
                                .to_vec()
                        })
                        .collect::<Vec<_>>();
