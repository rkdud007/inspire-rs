...
pub const DEF_MODULI: &[u64] = &[268369921u64, 249561089u64];
...
pub fn params_for_scenario_medium_payload(
    ...
    let working_num_large_items =
        (input_num_items as f64 * input_item_size_bits as f64 / (log_p * gamma_0) as f64).ceil();
    ...
    let (nu_1, nu_2) = if num_tiles_log2 % 2 == 0 {
        (
            (num_tiles_log2 / 2) + log_factor,
            (num_tiles_log2 / 2) - log_factor,
        )
    } else {
        (
            ((num_tiles_log2 + 1) / 2) + log_factor,
            ((num_tiles_log2 - 1) / 2) - log_factor,
        )
    };
    ...
    let modulus_log2 =
        (2. * 2. * (p as f64)).log2() + (2.0 * 41.0 * (2 as f64).ln()).sqrt().log2() + max_variance;
    // println!("best modulus_log2: {}", modulus_log2);

    pub const CUSTOM_MODULI: &[u64] = &[67043329u64, 132120577u64];
    let modulus: f64 = 67043329. * 132120577.;
    assert!(modulus.log2() >= modulus_log2);
    let mut params: Params =
        internal_params_for(poly_len, nu_1, 0, p, q2_bits, t_exp_left, CUSTOM_MODULI);
    params.instances = db_cols;
    (
        params,
        (
            1 << (nu_1 + poly_len_log2),
            1 << (nu_2 + poly_len_log2),
            log_p * gamma_0,
        ),
    )
}

pub fn params_for_scenario(
    input_num_items: usize,
    input_item_size_bits: usize,
) -> (Params, (usize, usize, usize)) {
    let poly_len_log2 = 11;
    ...
    let params = internal_params_for(poly_len, nu_1, nu_2, p, q2_bits, t_exp_left, DEF_MODULI);
    (
        params,
        (1 << (nu_1 + poly_len_log2), 1 << (nu_2 + poly_len_log2), 8),
    )
}

pub fn params_for_scenario_simplepir(
    input_num_items: usize,
    input_item_size_bits: usize,
) -> (Params, (usize, usize, usize)) {
    let poly_len_log2 = 11;
    ...
    let total_db_num_pts =
        (((input_num_items * input_item_size_bits) as f64) / log_p as f64).ceil() as usize; // -> total number of pt moduli required to represent the database
...
pub fn params_for_scenario_simplepir_small(
    input_num_items: usize,
    input_item_size_bits: usize,
) -> (Params, (usize, usize, usize)) {
    ...
    pub const CUSTOM_DEF_MODULI: &[u64] = &[268369921u64];

    let mut params =
        internal_params_for(poly_len, nu_1, 0, p, q2_bits, t_exp_left, CUSTOM_DEF_MODULI);
    params.instances = db_cols_poly;
    (params, (new_num_items, 1, new_item_size_num_pts * log_p))
}
