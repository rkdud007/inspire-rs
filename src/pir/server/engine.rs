...
impl Response {
    pub fn get_size_bytes(&self) -> usize {
        get_size_bytes_one(&self.packed_mask_mod_switched)
            + get_size_bytes_one(&self.packed_body_mod_switched)
    }
}
    ...
    u64: From<T>,
{
        ...
        I: Iterator<Item = T>,
    {
        ...
        let smaller_params = match protocol_type {
            ...
            ProtocolType::InsPIRe => {
                ...
                smaller_params.db_dim_2 = (((blowup_factor * params.poly_len as f64).ceil()
                    + (blowup_factor * this_gamma as f64).ceil())
                    / params.poly_len as f64)
                    .log2()
                    .ceil() as usize;
                smaller_params
    ...
    pub fn multiply_with_db_ring(
        ...
        // Parallelize over columns with rayon
        let per_col_results: Vec<Vec<u64>> = col_vec
            .par_iter()
            .map(|&col| {
                let mut prod = PolyMatrixNTT::zero(params, 1, 1);
                let mut db_elem_poly = PolyMatrixRaw::zero(params, 1, 1);
                let mut db_elem_ntt = PolyMatrixNTT::zero(params, 1, 1);
        ...
        let mut result = Vec::with_capacity(col_range.len() * params.poly_len);
        for col_result in per_col_results {
            result.extend(col_result);
        }

        let now = Instant::now();
        let res = transpose_generic(&result, col_range.len(), params.poly_len);
    ...
    pub fn generate_pseudorandom_query(&self, public_seed_idx: u8) -> Vec<PolyMatrixNTT> {
        ...
        let query = y_client.generate_query_impl(
            ...
            0,
        ); // TODO: is this correct type of packing?
    ...
    /// GPU-accelerated hint computation.
    /// Replaces answer_hint_ring with a fused NTT+multiply+accumulate GPU kernel.
    pub fn answer_hint_ring_gpu(&self, public_seed_idx: u8, cols: usize) -> Vec<u64> {
        let preprocessed_query = self.generate_pseudorandom_query(public_seed_idx);
    ...
    pub fn perform_offline_precomputation_simplepir(
        ...
        if online_only {
            ...
            for _ in 0..db_cols_prime {
                precomp_inspir_vec.push(PrecompInsPIR {
                    ...
                    bold_t_condensed: PolyMatrixNTT::random(
                        &params,
                        thing_size - 1,
                        params.t_exp_left,
                    ),
                    bold_t_bar_condensed: PolyMatrixNTT::random(
                        &params,
                        thing_size - 1,
                        params.t_exp_left,
                    ),
        ...
        // Determine if we can use the GPU-fused hint→prep_pack pipeline
        let use_gpu_prep_pack = self.second_level_packing_mask == PackingType::InspiRING
            && gamma > params.poly_len / 2
            && std::env::var("INSPIRE_NO_GPU").is_err();
        ...
        if use_gpu_prep_pack {
            ...
            // Run hint kernel — result stays on GPU (no D2H copy)
            cuda_hint::gpu_hint_compute_device_only(
                self.db_u16(),
                db_rows,
                db_cols,
                db_rows_poly,
            ...
            // Free query (no longer needed), keep hint output + NTT tables
            cuda_hint::gpu_hint_cleanup_partial();

            let t1 = Instant::now();

            // Run GPU prep_pack kernel (negacyclic_perm + forward NTT on device)
            let d_hint = cuda_hint::gpu_hint_device_output();
            let d_ntt = cuda_hint::gpu_hint_device_ntt_tables();
            crate::gpu::prep_pack::gpu_prep_pack_run(
                d_hint,
                d_ntt,
                db_cols,
                params.moduli[0],
            ...
            // Free hint output + NTT tables (prep_pack consumed them)
            cuda_hint::gpu_hint_cleanup_output();

            prepacked_lwe = Vec::new(); // not used in GPU path
        } else {
            // CPU path: hint with D2H, then CPU prep_pack
            let hint_0: Vec<u64> = self.answer_hint_ring_gpu(SEED_0, db_cols);
            let _ = t0.elapsed();

            let t1 = Instant::now();
            let combined = [&hint_0[..], &vec![0u64; db_cols]].concat();
            assert_eq!(combined.len(), db_cols * (params.poly_len + 1));
            prepacked_lwe = prep_pack_many_lwes(&params, &combined, db_cols_prime, gamma);
            let _ = t1.elapsed();
        }

        // hint_0 is poly_len x db_cols
        if let Some(measurement) = measurement {
            measurement.offline.simplepir_prep_time_ms = t0.elapsed().as_millis() as usize;
        }
        ...
        match self.second_level_packing_mask {
            PackingType::InspiRING => {
                // Setup persistent shared GPU data before parallel loop
                let use_gpu_setup =
                    gamma > params.poly_len / 2 && std::env::var("INSPIRE_NO_GPU").is_err();
                if use_gpu_setup {
                    setup_packing_gpu(&self.packing_params_set[&gamma]);
                }

                if use_gpu_prep_pack {
                    // GPU-fused path: packing kernel + automorphism + collapse on GPU
                    let d_prepacked_base =
                        crate::gpu::prep_pack::gpu_prep_pack_output_ptr() as usize;
                    let coeff_count = params.crt_count * params.poly_len;
                    let poly_len = params.poly_len;
                    let nphalf = poly_len / 2;

                    let use_gpu_collapse = std::env::var("INSPIRE_NO_GPU_COLLAPSE").is_err();

                    if use_gpu_collapse {
                        // ========== GPU COLLAPSE PATH ==========
                        // Step 1: Upload automorphism tables
                        let t_automorph = Instant::now();
                        let (d_tables, d_gen_pows, num_tables) =
                            crate::gpu::collapse::gpu_collapse_upload_automorph_tables(
                                &self.packing_params_set[&gamma],
                                &params,
                            );

                        // Step 2: Allocate r_all/r_bar_all on device
                        crate::gpu::collapse::gpu_collapse_alloc_r_all(
                            db_cols_prime,
                            nphalf,
                            coeff_count,
                        );

                        // Step 3: Per-column packing + automorphism
                        for i in 0..db_cols_prime {
                            let d_a_ct = (d_prepacked_base
                                + i * gamma * coeff_count * std::mem::size_of::<u64>())
                                as *const u64;

                            let _guard = crate::gpu::packing::GPU_MUTEX.lock().unwrap();
                            crate::gpu::packing::gpu_packing_precompute_fast_device_no_d2h(d_a_ct);
                            crate::gpu::collapse::gpu_collapse_automorph_col(
                                i,
                                nphalf,
                                coeff_count,
                                poly_len,
                        ...
                        let _ = t_automorph.elapsed();

                        // Step 4: Free packing scratch and prep_pack output
                        crate::gpu::prep_pack::gpu_prep_pack_cleanup();

                        // Step 5: Setup collapse weights and run
                        ...
                        crate::gpu::collapse::gpu_collapse_setup_weights(
                            w_all,
                            w_bar_all,
                            v_mask,
                            &params,
                        ...
                        crate::gpu::collapse::gpu_collapse_run_kernel(&params, db_cols_prime);

                        // Step 6: Download a_hat and build PrecompInsPIR
                        let a_hat_flat = crate::gpu::collapse::gpu_collapse_download_a_hat_rs(
                            db_cols_prime,
                            poly_len,
                        );

                        precomp_inspir_vec = (0..db_cols_prime)
                            .map(|i| {
                                let mut a_hat = PolyMatrixRaw::zero(&params, 1, 1);
                                a_hat
                                    .as_mut_slice()
                                    .copy_from_slice(&a_hat_flat[i * poly_len..(i + 1) * poly_len]);
                                PrecompInsPIR::with_a_hat_only(&params, a_hat)
                            })
                            .collect();

                        let _ = t_collapse.elapsed();

                        // Step 7: Have online kernel adopt device-resident bold_t
                        crate::gpu::collapse::gpu_collapse_adopt_bold_t_for_online(
                            &params,
                            db_cols_prime,
                        );

                        // Step 8: Free working buffers (r_all, w_all, etc.) — keep bold_t
                        crate::gpu::collapse::gpu_collapse_cleanup_working();
                    } else {
                        // Original per-column GPU path (with D2H and CPU collapse)
                        let results: Vec<_> = (0..db_cols_prime)
                            ...
                            .map(|i| {
                                ...
                                full_packing_with_preprocessing_offline_device(
                                    &self.packing_params_set[&gamma],
                                    &w_all,
                                    &w_bar_all,
                                    &v_mask,
                        ...
                        // Free GPU prep_pack output
                        crate::gpu::prep_pack::gpu_prep_pack_cleanup();
                    }
                } else {
                    // CPU prep_pack path: build a_ct_tilde from host data
                    let results: Vec<_> = (0..db_cols_prime)
                        .into_par_iter()
                        .map(|i| {
                ...
                if use_gpu_setup {
                    crate::gpu::packing::gpu_packing_teardown_shared();
                }
            }
            PackingType::CDKS => {
                for i in 0..db_cols_prime {
        ...
        let _ = t3.elapsed();
        debug!("Precomp in {} us", t0.elapsed().as_micros());
    ...
    pub fn perform_offline_precomputation(
        ...
        if online_only {
            ...
            for _ in 0..db_cols * out_rows {
                smaller_db.push((rng_pub.random::<u64>() % params.pt_modulus) as u16);
            }
            ...
            for _ in 0..params.poly_len * out_rows {
                hint_1.push(rng_pub.random::<u64>() % params.modulus);
            }
    ...
    pub fn perform_offline_precomputation_medium_payload(
        ...
        let mask_num_packed_rlwes =
            (mask_decomposed_num_coeffs as f64 / gammas[1] as f64).ceil() as usize;

        if online_only {
            let mut rng_pub = ChaCha20Rng::from_seed(get_seed(SEED_0));

            let mut hint_1: Vec<u64> =
                Vec::with_capacity(params.poly_len * smaller_db_total_num_cols_rounded_up);
            for _ in 0..params.poly_len * smaller_db_total_num_cols_rounded_up {
                hint_1.push(rng_pub.random::<u64>() % params.modulus);
            }

            let mut rng_pub = ChaCha20Rng::from_seed(get_seed(SEED_1));
            let mut smaller_db: Vec<u16> =
                Vec::with_capacity(smaller_db_total_num_cols_rounded_up * db_cols_prime);
            ...
            assert_eq!(
                smaller_db.len(),
                smaller_db_total_num_cols_rounded_up * db_cols_prime
            );
            let smaller_server: YServer<u16> = YServer::<u16>::new_small(
                &self.smaller_params,
                smaller_db.into_iter(),
            ...
            let thing_size = if gammas[0] == params.poly_len {
                params.poly_len / 2
            } else {
                gammas[0]
            };
            ...
            let mut precomp_inspir_vec = Vec::new();
            let thing_size = if gammas[1] == params.poly_len {
                params.poly_len / 2
            } else {
                gammas[1]
            };
            ...
            let offline_packing_keys = if gammas.len() > 2 {
                if gammas[2] <= params.poly_len / 2 {
                    ...
                    OfflinePackingKeys::init_full(
                        &self.packing_params_set[&gammas[2]],
                        W_SEED,
                        V_SEED,
                    )
                }
        ...
        let prepacked_lwe =
            prep_pack_many_lwes(&params, &combined, db_cols_prime, gamma_first_layer);
        ...
        let offline_packing_key_first_layer =
            offline_packing_keys_set.get(&gamma_first_layer).unwrap();
        for i in 0..db_cols_prime {
            let mut a_ct_tilde = Vec::new();
        ...
        let new_hint = precomp_inspir_vec_first_layer
            ...
            .map(|x| {
                [
                    x.a_hat.get_poly(0, 0).to_vec(),
                    vec![0u64; gamma_first_layer],
                ]
                .concat()
            })
            .collect::<Vec<_>>()
        ...
        let intermediate_cts_rescaled_transposed = transpose_generic(
            &intermediate_cts_rescaled,
            db_cols_prime,
        ...
        assert_eq!(
            hint_1.len(),
            params.poly_len * smaller_db_total_num_cols_rounded_up
        );
        ...
        let combined = [
            &hint_1[..],
            &vec![0u64; smaller_db_total_num_cols_rounded_up],
        ]
        .concat();
        assert_eq!(
            combined.len(),
            smaller_db_total_num_cols_rounded_up * (params.poly_len + 1)
        );
        ...
        let offline_packing_key_second_layer_mask =
            offline_packing_keys_set.get(&gammas[1]).unwrap();
        let mut precomp_inspir_vec = Vec::new();
        for i in 0..mask_num_packed_rlwes {
            ...
            let tup_inspir = if gamma_second_layer_mask <= params.poly_len / 2 {
                let w_all = offline_packing_key_second_layer_mask
                    .w_all
                    .as_ref()
                    .unwrap();
                packing_with_preprocessing_offline(
                ...
                let w_all = offline_packing_key_second_layer_mask
                    ...
                    .as_ref()
                    .unwrap();
                let w_bar_all = offline_packing_key_second_layer_mask
                    .w_bar_all
                    .as_ref()
                    .unwrap();
                let v_mask = offline_packing_key_second_layer_mask
                    .v_mask
                    .as_ref()
                    .unwrap();
                full_packing_with_preprocessing_offline(
    ...
    pub fn perform_online_computation(
        ...
        for (intermediate_chunk, packed_query_col) in intermediate
            .as_slice()
            .chunks(db_cols)
            .zip(second_dim_queries.iter())
        {
            ...
            let mut packed = match self.second_level_packing_mask {
                ...
                PackingType::NoPacking => {
                    unreachable!("NoPacking in mask packing");
    ...
    pub fn perform_online_computation_medium_payload(
        ...
        let mask_num_packed_rlwes =
            (mask_decomposed_num_coeffs as f64 / gamma_second_layer_mask as f64).ceil() as usize;
        ...
        let packing_key_rotations_time = Instant::now();
        packing_keys_set
            .get_mut(&gamma_first_layer)
            .unwrap()
            .expand();
        packing_keys_set
            .get_mut(&gamma_second_layer_mask)
            .unwrap()
            .expand();
        if self.second_level_packing_body == PackingType::InspiRING {
        ...
        assert_eq!(
            intermediate_cts_rescaled.len(),
            db_cols_prime * gamma_first_layer
        );

        let intermediate_cts_rescaled_transposed =
            transpose_generic(&intermediate_cts_rescaled, db_cols_prime, gamma_first_layer);
        ...
        assert_eq!(
            part_to_add.len(),
            body_decomposed_num_coeffs * db_cols_prime
        );
        ...
        assert_eq!(
            secondary_hint.len(),
            params.poly_len * body_decomposed_num_coeffs
        );
        ...
        match self.second_level_packing_body {
            ...
            PackingType::InspiRING => {
                let packed_online_num = (body_decomposed_num_coeffs as f64
                    / gamma_second_layer_body as f64)
                    .ceil() as usize;
                for i in 0..packed_online_num {
                    ...
                    for j in 0..gamma_second_layer_body {
                        if i * gamma_second_layer_body + j < body_decomposed_num_coeffs {
                            a_ct_tilde.push(
                                excess_cts[i * gamma_second_layer_body + j]
                                    .submatrix(0, 0, 1, 1)
                                    .ntt(),
                            );
                            b_poly.get_poly_mut(0, 0)[j] =
                                excess_cts[i * gamma_second_layer_body + j].get_poly(1, 0)[0];
                        }
                    }
        ...
        for ct in packed_mask.iter() {
            let res_switched =
                ct.switch_and_keep(rlwe_q_prime_1, rlwe_q_prime_2, gamma_second_layer_mask);
        ...
        for ct in packed_body.iter() {
            let res_switched =
                ct.switch_and_keep(rlwe_q_prime_1, rlwe_q_prime_2, gamma_second_layer_body);
