        ...
        .map(|instance| {
            ...
            for trial in 0..(params.n * params.n) {
                ...
                if instance == 0 && trial == 0 {
                    unsafe {
                        let client_test = &raw const CLIENT_TEST;
                        if let Some((sk_reg, target)) = (*client_test).as_ref() {
...
mod test {
    ...
    fn dec_gsw<'a>(params: &'a Params, ct: &PolyMatrixNTT, client: &mut Client) -> u64 {
        ...
        if i64::abs(val) < (1i64 << 10) { 0 } else { 1 }
    }
    ...
    fn coefficient_expansion_is_correct() {
        ...
        let mut rng_pub = ChaCha20Rng::from_seed(rand::random::<[u8; 32]>());
