    ...
    *const T: ToM512,
{
    ...
    assert_eq!(
        a.len(),
        a_elems,
        "For K=1, 'a' should have a_elems elements."
    );
    ...
    let k_chunk_size_factor = if simd_width > 0 {
        a_elems / simd_width
    } else {
        0
    };
    ...
    let num_chunks = if chunk_size > 0 {
        k_chunk_size_factor / chunk_size
    } else {
        0
    };
        ...
        for k_outer in 0..num_chunks {
            for j_outer in 0..j_num_chunks {
                ...
                for k_inner in 0..chunk_size {
                    ...
                    total_sum_lo_val =
                        _mm512_add_epi64(total_sum_lo_val, _mm512_mul_epu32(a_val_lo, b_val_simd));
                    total_sum_hi_val =
                        _mm512_add_epi64(total_sum_hi_val, _mm512_mul_epu32(a_val_hi, b_val_simd));
                }
...
mod test {
    ...
    fn test_fast_batched_dot_product_avx512() {
        ...
        for _ in 0..trials {
            ...
            let b_u16_slc = unsafe {
                std::slice::from_raw_parts(b_u64.as_ptr() as *const u16, b_u64.len() * 4)
            };
