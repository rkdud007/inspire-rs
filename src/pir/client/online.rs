...
pub fn generate_matrix_ring(
    ...
    for i in 0..rows_outer {
        for j in 0..cols_outer {
            ...
            for idx in 0..n {
                a[idx] = rng_pub.sample::<u32, _>(rand::distr::StandardUniform);
            }
...
impl<'a> YClient<'a> {
    ...
    /// Fast path: only extract the b values (last row) from RLWE ciphertexts.
    /// Avoids building the full negacyclic matrix which is O(N^2) per RLWE.
    pub fn generate_query_b_values(
        &self,
        public_seed_idx: u8,
        dim_log2: usize,
        packing_type: PackingType,
        ...
        for k in 0..num_rlwes {
            let b_poly = rlwes[k].get_poly(1, 0);
            b_row.extend_from_slice(&b_poly[..how_many]);
        }
        b_row
    }
