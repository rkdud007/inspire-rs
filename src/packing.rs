        ...
        for i in 0..a.rows {
            for j in 0..a.cols {
                ...
                for z in 0..params.poly_len {
                    ...
                    // TODO: Does this assume that the digit decomposition is exactly 2?
                }
            }
        }
        res
...
#[cfg(not(target_arch = "x86_64"))]
pub fn multiply_poly_avx(_params: &Params, _res: &mut [u64], _a: &[u64], _b: &[u64]) {
    unimplemented!("x86_64 required");
}

#[cfg(target_arch = "x86_64")]
pub fn multiply_poly_avx_in_range(
    _params: &Params,
    res: &mut [u64],
    a: &[u64],
    b: &[u64],
...
pub fn pack_many_lwes_inspir<'a>(
    ...
    let res: Vec<PolyMatrixRaw<'a>> = (0..num_rlwe_outputs)
        ...
        .map(|i| {
            ...
            if gamma <= packing_params.params.poly_len / 2 {
...
pub fn apply_automorph_ntt<'a>(
    ...
    for i in 0..mat.rows {
        for j in 0..mat.cols {
            ...
            let mut res_poly: Vec<&mut [u64]> = res
                .get_poly_mut(i, j)
                .chunks_exact_mut(params.poly_len)
                .collect();
...
pub fn apply_automorph_ntt_double<'a>(
    ...
    for i in 0..mat.rows {
        for j in 0..mat.cols {
            let poly = mat.get_poly(i, j);
            let mut res_1_poly: Vec<&mut [u64]> = res_1
                .get_poly_mut(i, j)
                .chunks_exact_mut(params.poly_len)
                .collect();
            let mut res_2_poly: Vec<&mut [u64]> = res_2
                .get_poly_mut(i, j)
                .chunks_exact_mut(params.poly_len)
                .collect();
...
impl PackParams<'_> {
    pub fn new<'a>(params: &'a Params, num_to_pack: usize) -> PackParams<'a> {
        ...
        for i in 0..params.poly_len {
            gen_pows.push(exponentiate_uint_mod(
                generator as u64,
                i as u64,
                2 * params.poly_len as u64,
            ) as usize);
        }
...
impl PackingKeys<'_> {
    ...
    pub fn expand<'a>(&mut self) {
        ...
        if !self.expanded {
            ...
            if self.full_key {
                let (x, y) = generate_rotations_double(
                    &packing_params,
                    &self.y_body_condensed.as_ref().unwrap(),
                );
                (self.y_all_condensed, self.y_bar_all_condensed) = (Some(x), Some(y));
            } else {
                let x =
                    generate_rotations(&packing_params, &self.y_body_condensed.as_ref().unwrap());
...
pub fn packing_with_preprocessing_offline<'a>(
    ...
    debug!(
        "Gadget inversion for R took {} us",
...
pub fn packing_with_preprocessing_offline_without_rotations<'a>(
    ...
    for i in (0..num_to_pack - 1).rev() {
        ...
        apply_automorph_ntt(
            ...
            &gadget,
            &mut rotated_gadget,
            gen_pows[(params.poly_len - i) % params.poly_len],
        );
...
pub fn packing_with_preprocessing_online<'a>(
    ...
    debug!(
        "Multiplication and addition for R took {} us",
        now.elapsed().as_micros()
    );
...
/// Upload shared packing data to GPU once (mono_table, idx, idx_bar, mod_inv).
...
pub fn setup_packing_gpu(packing_params: &PackParams) {
    ...
    let non_zeros = num_to_pack; // gamma = poly_len for 32GB
    let coeff_count = params.crt_count * params.poly_len;
    let two_n = 2 * params.poly_len;

    // Build mono_table
    let mut mono_table = vec![0u64; two_n * coeff_count];
    for idx in 0..params.poly_len {
        let src = monomial_ntts[idx].get_poly(0, 0);
        mono_table[idx * coeff_count..(idx + 1) * coeff_count].copy_from_slice(src);
    }
    for idx in 0..params.poly_len {
        let src = neg_monomial_ntts[idx].get_poly(0, 0);
        let off = params.poly_len + idx;
        mono_table[off * coeff_count..(off + 1) * coeff_count].copy_from_slice(src);
    }

    // Build idx tables
    let mut idx_table = vec![0i32; num_to_pack_half * non_zeros];
    let mut idx_bar_table = vec![0i32; num_to_pack_half * non_zeros];
    for i in 0..num_to_pack_half {
        let gen_pow_i = gen_pows[(params.poly_len - i) % params.poly_len];
        for j in 0..non_zeros {
            let index = (j * gen_pow_i) % two_n;
            idx_table[i * non_zeros + j] = if index < params.poly_len {
                index as i32
            } else {
                (params.poly_len + (index % params.poly_len)) as i32
            };

            let index_bar = (two_n - (j * gen_pow_i) % two_n) % two_n;
            idx_bar_table[i * non_zeros + j] = if index_bar < params.poly_len {
                index_bar as i32
            } else {
                (params.poly_len + (index_bar % params.poly_len)) as i32
            };
        }
    }

    let mod_inv_flat: Vec<u64> = mod_inv_poly.get_poly(0, 0).to_vec();

    crate::gpu::packing::gpu_packing_setup_shared(
        &mono_table,
        &idx_table,
        &idx_bar_table,
        &mod_inv_flat,
        ...
        params.poly_len,
        params.crt_count,
        &params.moduli[..params.crt_count],
    );
}
...
fn full_packing_with_preprocessing_offline_inner<'a>(
    ...
    let non_zeros = if let Some((_, nz)) = device_a_ct {
        nz
    } else {
        a_ct_tilde.len()
    };

    let use_gpu = cfg!(feature = "gpu") && std::env::var("INSPIRE_NO_GPU").is_err();
    #[allow(unused_variables)]
    let coeff_count = params.crt_count * params.poly_len;

    #[allow(unused_variables)]
    let (mut r_all, mut r_bar_all) = if use_gpu {
        #[cfg(feature = "gpu")]
        {
            // Check if we have device-resident a_ct from GPU prep_pack
            let (r_flat, r_bar_flat) = if let Some((d_a_ct, _)) = device_a_ct {
                if crate::gpu::packing::is_packing_setup() {
                    // Device fast path: a_ct already on GPU, no H2D needed
                    let _guard = crate::gpu::packing::GPU_MUTEX.lock().unwrap();
                    crate::gpu::packing::gpu_packing_precompute_fast_device(
                        d_a_ct,
                        num_to_pack_half,
                        coeff_count,
                    )
                } else {
                    panic!("device_a_ct provided but packing setup not done");
                }
            } else {
                // GPU path: batch multiply-accumulate
                let mut a_ct_flat = vec![0u64; non_zeros * coeff_count];
                for j in 0..non_zeros {
                    let src = a_ct_tilde[j].get_poly(0, 0);
                    a_ct_flat[j * coeff_count..(j + 1) * coeff_count].copy_from_slice(src);
                }

                if crate::gpu::packing::is_packing_setup() {
                    // Fast path: shared data already on GPU, only upload a_ct
                    let _guard = crate::gpu::packing::GPU_MUTEX.lock().unwrap();
                    crate::gpu::packing::gpu_packing_precompute_fast(
                        &a_ct_flat,
                        num_to_pack_half,
                        coeff_count,
                    )
                } else {
                    // Slow path: upload everything (first call or no setup)
                    let two_n = 2 * params.poly_len;
                    let mut mono_table = vec![0u64; two_n * coeff_count];
                    for idx in 0..params.poly_len {
                        let src = monomial_ntts[idx].get_poly(0, 0);
                    ...
                    crate::gpu::packing::gpu_packing_precompute(
                        &mono_table,
                        &a_ct_flat,
                        &idx_table,
                        &idx_bar_table,
            ...
            // 6. Reconstruct PolyMatrixNTT and apply automorphisms (CPU, fast)
            let mut r_all = Vec::with_capacity(num_to_pack_half);
            let mut r_bar_all = Vec::with_capacity(num_to_pack_half);
            for i in 0..num_to_pack_half {
                // r result -> apply automorphism
                let mut r_ntt = PolyMatrixNTT::zero(&params, 1, 1);
                r_ntt
                    .as_mut_slice()
                    .copy_from_slice(&r_flat[i * coeff_count..(i + 1) * coeff_count]);

                let mut r_rotated = PolyMatrixNTT::zero(&params, 1, 1);
                apply_automorph_ntt(&params, &tables, &r_ntt, &mut r_rotated, gen_pows[i]);
                r_all.push(r_rotated);

                // r_bar result -> apply automorphism
                let mut r_bar_ntt = PolyMatrixNTT::zero(&params, 1, 1);
                r_bar_ntt
                    .as_mut_slice()
                    .copy_from_slice(&r_bar_flat[i * coeff_count..(i + 1) * coeff_count]);

                let mut r_bar_rotated = PolyMatrixNTT::zero(&params, 1, 1);
                apply_automorph_ntt(
                    &params,
                    &tables,
                    &r_bar_ntt,
    ...
    let _ = (_t_start.elapsed(), non_zeros, num_to_pack_half, use_gpu);
    let _t_collapse = Instant::now();
    ...
    // Pre-allocate scratch for gadget_invert (Optimization C)
    let mut gadget_scratch = PolyMatrixRaw::zero(&params, 1, params.t_exp_left);

    for i in (0..num_to_pack_half - 1).rev() {
        gadget_invert_transposed_into(&mut gadget_scratch, &r_all[i + 1].raw());
        let gadget = gadget_scratch.ntt();
        ...
        gadget_invert_transposed_into(&mut gadget_scratch, &r_bar_all[i + 1].raw());
        let gadget = gadget_scratch.ntt();
    ...
    gadget_invert_transposed_into(&mut gadget_scratch, &r_bar_all[0].raw());
    let bold_t_hat = gadget_scratch.ntt();
...
pub fn full_packing_with_preprocessing_offline_without_rotations<'a>(
    ...
    let mut bold_t_bar_g_prime =
        PolyMatrixNTT::zero(&params, num_to_pack_half - 1, params.t_exp_left);
    ...
    for i in (0..num_to_pack_half - 1).rev() {
        ...
        apply_automorph_ntt(
            ...
            &gadget,
            &mut rotated_gadget,
            2 * params.poly_len - gen_pows[(params.poly_len - i) % params.poly_len],
        );
...
pub fn full_packing_with_preprocessing_online_without_rotations<'a>(
    ...
    for i in 0..num_to_pack_half - 1 {
        ...
        apply_automorph_ntt(
            ...
            &temp_poly,
            &mut rotated_temp,
            2 * params.poly_len - packing_params.gen_pows[i],
        );
...
pub fn query_gen<'a>(
    ...
    let y_body = generate_ksk_body(&params, &sk_reg, generator, &w_mask, rng_y);
...
mod test {
    ...
    use crate::pir::{
        engine::generate_y_constants,
        params::params_for_scenario,
        scheme::{V_SEED, W_SEED},
    };
    ...
    fn test_packing() {
        ...
        println!(
            "rescaled: {:?}",
            &rescaled.as_slice()[..num_to_pack.min(50)]
        );
        assert_eq!(
            rescaled.as_slice()[..num_to_pack],
            gold.as_slice()[..num_to_pack]
        );
    }
    ...
    fn test_precompute_packing() {
        ...
        // &w_mask, &mut ChaCha20Rng::from_seed(rand::random::<[u8; 32]>())); let z_body =
        // generate_ksk_body(&params, sk_reg, 2*params.poly_len - 1, &v_mask, &mut
        // ChaCha20Rng::from_seed(rand::random::<[u8; 32]>()));
        let mut b_poly = PolyMatrixRaw::zero(&params, 1, 1);
        for i in 0..params.poly_len {
    ...
    fn test_packing_with_preprocessing() {
        ...
        println!(
            "Offline packing took {} us",
            now_offline.elapsed().as_micros()
        );
        ...
        let packed = if max_num_to_pack <= params.poly_len / 2 {
            ...
            println!(
                "Prerotations took {} us",
                now_prerotate.elapsed().as_micros()
            );
            packing_with_preprocessing_online(
                &packing_params,
                &precomp_inspiring,
            ...
            let now_prerotate = Instant::now();
            let (y_all_condensed, y_bar_all_condensed) =
                generate_rotations_double(&packing_params, &y_condensed);
            println!(
                "Prerotations took {} us",
                now_prerotate.elapsed().as_micros()
            );
        ...
        println!(
            "Online packing took {} us",
            now_online.elapsed().as_micros()
        );
    ...
    fn test_polynomial_mult() {
        ...
        for j in 0..t {
            p3.get_poly_mut(j, 0)
                .copy_from_slice(&p2.get_poly(j, 0)[..]);
        }
        ...
        for _ in 0..1000 {
            ...
            for k in 0..t {
                ...
                multiply_add_poly(&params, res_poly, poly1, poly2);
            }
    ...
    use crate::pir::params::params_for_scenario_simplepir_small;
    ...
    fn test_new_mult() {
        ...
        assert_eq!(
            res1.raw().get_poly(0, 0)[..10],
            res2.raw().get_poly(0, 0)[..10]
        );
    }
}
